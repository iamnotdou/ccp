---
title: Architecture
description: How Hedera, Ledger, and ENS form the three layers of the CCP stack.
---

## Three-Layer Stack

CCP integrates three technologies, each serving a distinct layer:
...
CCP's containment certificates need to be queried at machine speed. When Agent B receives a payment request from Agent A, it must verify the certificate in milliseconds. Hedera's 3s finality and native `view` call performance make real-time verification practical.
...
CCP's thesis distinguishes agent-independent from agent-influenceable containment. The strongest form of agent-independent containment is hardware enforcement. An LLM agent can social-engineer a human operator, but it cannot social-engineer a hardware security module.
