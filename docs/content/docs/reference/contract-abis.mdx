...
| `revokeForCause(bytes32 certHash)` | `bytes32` | - | write (ChallengeManager only) |
...
| `updateConfig(...)` | config params | - | write (Ledger only) |
...
| Function | Inputs | Returns | Mutability |
|----------|--------|---------|------------|
| `challenge(bytes32 certHash, uint8 challengeType, bytes calldata evidence)` | `bytes32, uint8, bytes` | `uint256 challengeId` | write |
