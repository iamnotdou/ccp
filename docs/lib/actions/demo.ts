...
async function getOrCreateCertHash(): Promise<{ certHash: Hash; isExisting: boolean }> {
  ...
  try {
    ...
    const [maxSingle, maxPeriodic, cosignThreshold, ledgerCosigner, reserveBalance, reserveAdequate, reserveLocked] =
      await Promise.all([
        ...
        publicClient.readContract({ address: addresses.reserveVault, abi: ReserveVaultABI, functionName: "isAdequate", args: [CONTAINMENT_BOUND, 30000] }),
...
export async function demoPhase2_PublishCert(): Promise<{
  txHash?: string;
  error?: string;
  certHash?: string;
  isValid?: boolean;
...
export async function demoPhase4_SmallPayment(): Promise<{
  ...
  try {
    ...
    return { txHash, spent: formatUnits(spent, 6), limit: formatUnits(limit, 6) };
  } catch (e: any) {
...
export async function demoPhase5_LargePayment(): Promise<{
  ...
  try {
    ...
    const cosig = await ledgerAccount.signMessage({ message: { raw: txHashToSign } });
...
export async function demoPhase7_Timeline(): Promise<{
  events?: Array<{ type: string; timestamp: string; details: Record<string, string> }>;
  error?: string;
}> {
  try {
