...
This is the Sybil problem applied to credit: the cost of acquiring a fresh identity must exceed the benefit of escaping a damaged one. For software agents, spinning up a new instance is nearly costless. As observed in the Moltbook agent community — one of the most active grassroots efforts to build agent credit systems — "agents can just stop existing." That is a failure mode with no analogue in human credit.
...
What started as a question about "trust" turns out to be a compounded probability problem. Let us state it formally, with appropriate honesty about what the math does and does not do.
...
**The agent can degrade its own containment.** The agent's action space includes "attempt to modify, circumvent, or weaken containment." Containment quality is endogenous to agent behavior: `p_i(t+1) = f(p_i(t), agent_actions(t))`.
...
## 10. Strategic Incentive Model

The containment framework implicitly assumes that participants will behave as designed. But a trust protocol is a multi-player game. If the incentives are wrong, rational actors will defect — and the protocol fails regardless of its technical soundness. This section makes the game-theoretic structure explicit.

### 10.1 Actors, Strategies, and Target Equilibrium

The CCP ecosystem has four principal actor types, each with distinct strategy spaces:

| Actor | Cooperative Strategy | Defection Strategy |
|---|---|---|
| **Operator** | Invest in real containment, fund reserves, obtain honest audits | Obtain fraudulent attestation, underfund reserves, exploit stale certificates |
| **Auditor** | Rigorously verify containment, stake reputation on findings | Rubber-stamp attestations for fees, collude with operators |
| **Counterparty (Verifier)** | Evaluate certificates against genuine risk policy, reject inadequate containment | Accept any certificate to maximize transaction volume, ignore containment quality |
| **Protocol Integrator** | Set meaningful admission thresholds, enforce certificate requirements | Set trivially low thresholds, creating a false sense of security |

The **target equilibrium** is a separating equilibrium: well-contained agents are cheaply distinguishable from poorly-contained agents, and the cost of faking containment exceeds the benefit.

### 10.2 Incentive Compatibility Conditions

For the protocol to be self-enforcing, the following must hold:

1. **Operator honesty constraint:** The cost of obtaining and maintaining genuine containment must be lower than the expected loss from fraudulent certification (detection probability × penalty). This requires that detection is credible and penalties are meaningful — reserve forfeiture, certificate revocation, and reputational damage to a persistent operator identity.

2. **Auditor integrity constraint:** The long-term value of an auditor's reputation must exceed the short-term gain from fraudulent attestation. This is a repeated-game condition — it holds only if auditors are durable identities with observable track records.

3. **Verifier rationality:** Counterparties must have access to sufficient information to distinguish certificate quality. The certificate schema and on-chain verifiability serve this function — but only if the information is actually used. Standard policy templates (Section 10.5) lower the cost of evaluation.

### 10.3 Why This Matters for Protocol Design

Without explicit incentive analysis, the protocol risks becoming a compliance checkbox — certificates are issued, published, and verified, but no actor has a strong reason to ensure they reflect reality. The game-theoretic lens identifies where the protocol needs *hard constraints* (reserve lock-up, on-chain enforcement) versus where it can rely on *repeated-game incentives* (auditor reputation, operator track record).

---

## 11. Risk Reduction Attribution

The containment bound tells a counterparty the worst-case loss. But it does not answer a more granular question: **which specific controls are actually creating the safety?** This matters for three reasons:

1. **Audit prioritization:** If a formally verified smart contract contributes 80% of the risk reduction and a human oversight layer contributes 5%, audit resources should be allocated accordingly.

2. **Architecture optimization:** Operators designing containment stacks need to know which layers are load-bearing and which are marginal.

3. **Insurance pricing:** Residual absorption should be priced based on which controls are actually effective, not just how many exist.

### 11.1 Shapley-Style Control Contribution

The conceptual framework is Shapley value attribution from cooperative game theory. Each containment layer is a "player" in a coalition game where the payoff is risk reduction. The Shapley value of a layer is its average marginal contribution to risk reduction across all possible orderings of layers.

For a containment architecture with layers {L1, L2, ..., Ln}:

> `φ_i = Σ [|S|!(n-|S|-1)!/n!] × [R(S ∪ {i}) - R(S)]`

Where R(S) is the risk with only the layers in coalition S active.

In practice, exact Shapley computation is intractable for large layer counts. But approximate methods — Monte Carlo sampling of coalition orderings, or simplified models that account for the agent-independent/agent-influenceable distinction — can yield actionable attribution reports.

### 11.2 Practical Output

A **control contribution report** for each certificate could include:

- Percentage of risk reduction attributable to each agent-independent layer
- Percentage attributable to each agent-influenceable layer
- Marginal contribution of the reserve (how much additional risk is absorbed by the economic backstop)
- Identification of redundant layers (layers whose removal would not materially increase risk)
- Identification of critical layers (layers whose removal would cause a discontinuous increase in risk)

This is a research-track feature, not an MVP requirement. But the framework should be designed to accommodate it — the certificate schema should capture enough structural information about layer dependencies to enable attribution analysis.
...
**Systemic concentration as a coordination game.** The systemic risk problem is worse than it appears. 1,000 valid certificates that all depend on the same auditor and same enforcement contract are not 1,000 independent safety proofs — they are one common failure domain repeated 1,000 times. This is a coordination-risk game where individual rationality (each operator choosing the cheapest reputable auditor) produces collective fragility. Making concentration visible — dependency graphs, common-auditor exposure metrics, shared-contract counts — transforms this from a hidden systemic risk into a priced one. Verifiers who can see concentration can diversify their exposure; the market can reward architectural diversity.

**The containment-utility tradeoff.** Tighter containment reduces the agent's utility. An agent with a $1 daily spending limit is very safe but useless. The optimization — maximize agent utility subject to R(t) ≤ acceptable risk — is where the real engineering decisions live.

**Cross-domain portability.** An agent's containment certification on Ethereum says nothing to Solana, Avalanche, or Visa's TAP. A universal standard for expressing containment quality — portable across chains, protocols, and off-chain systems — does not yet exist.
...
**Risk reduction attribution.** Determining the marginal safety contribution of each containment layer — particularly under correlated failure and agent-induced degradation — requires formal methods that do not yet exist in production-ready form. Shapley-style attribution provides the conceptual framework, but tractable approximations for heterogeneous containment stacks remain an open research problem.
...
- Coinbase, x402 protocol v2, December 18, 2025. https://docs.cdp.coinbase.com/get-started/changelog
